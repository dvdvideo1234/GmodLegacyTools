...
function ENT:Draw()
  ...
  if(tr.Entity == self.Entity) then
    AddWorldTip(self.Entity:EntIndex(),
                self.Entity:GetNetworkedString("label"),
                0.5, self.Entity:GetPos(), self.Entity)
  end

  self.Range = self.Entity:GetNWInt("range")

  local tr = util.QuickTrace(self.Entity:GetPos() +
                             self.Entity:GetForward() * 5,
                             self.Entity:GetForward() * self.Range, self.Entity)

  render.SetMaterial(mat)
  render.DrawBeam(self.Entity:GetPos(), tr.HitPos, 1, 0, 10)
